...
class TutorSettings
{
    ...
    public function tutorlms_settings($settings)
    {
        ...
        if ( ! defined('MAILOPTIN_DETACH_LIBSODIUM')) :
            ...
            if ( ! empty($saved_connections) && $saved_connections != 'leadbank') {
                ...
                $lists                                                     = TutorInit::get_instance()->tutorlms_select_list_options($saved_connections);
                ...
                if ( ! empty($saved_lists)) {
                    $mappable_fields = TutorInit::get_instance()->merge_vars_field_map($saved_connections, $saved_lists);
    ...
    public function process_submission($course_id, $user_id)
    {
        ...
        foreach (TutorInit::get_instance()->merge_vars_field_map($connection_service, $connection_email_list) as $key => $value) {
            $mapped_key      = rawurlencode('mailoptin_tutorlms_mapped_fields_' . $key);
