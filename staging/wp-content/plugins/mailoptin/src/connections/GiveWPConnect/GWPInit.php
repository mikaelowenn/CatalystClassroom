...
class GWPInit
{
    ...
    public function fetch_lists()
    {
        ...
        if ( ! empty($connection) && $connection != 'leadbank') {
            $lists       = $this->gwp_select_list_options($connection);
     ...
     * @return array
     */
    public function gwp_select_list_options($saved_integration)
    {
        $lists = [];
    ...
    public function process_submission($payment)
    {
        ...
        foreach (GWPInit::get_instance()->merge_vars_field_map($connection_service, $connection_email_list) as $key => $value) {
