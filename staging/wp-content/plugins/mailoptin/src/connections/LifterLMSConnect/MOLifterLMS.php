...
class MOLifterLMS extends \LLMS_Abstract_Integration
{
    ...
    private function per_course_subscription($user_id, $post_id)
    {
        ...
        foreach (Connect::merge_vars_field_map($connection, $connection_email_list) as $key => $value) {
    ...
    private function global_subscription($user_id)
    {
        ...
        foreach (Connect::merge_vars_field_map($connection_service, $connection_email_list) as $key => $value) {
        ...
        if (defined('MAILOPTIN_DETACH_LIBSODIUM')) {
            ...
            if ( ! empty($saved_connection)) {
                ...
                if ( ! empty($saved_lists)) {
                    ...
                    $field_map = Connect::merge_vars_field_map($saved_connection, $saved_lists);
                    ...
                    if ( ! empty($field_map)) {
                        ...
                        foreach (Connect::merge_vars_field_map($saved_connection, $saved_lists) as $key => $value) {
                            $mapped_key = rawurlencode('mo_llms_list_mapped_fields_' . $key);
