...
class UserRegisterInit
{
    ...
    public function process_submission($user_id)
    {
        ...
        foreach (self::get_instance()->merge_vars_field_map($connection_service, $connection_email_list) as $key => $value) {
     ...
     * @return array
     */
    public function user_registration_optin_select_list_options($saved_integration)
    {
        $lists = [];
        if ( ! empty($saved_integration) && $saved_integration != 'leadbank') {
