...
class Membership
{
    ...
    public function membership_level_settings($level)
    {
        ...
        $lists      = PMPROInit::get_instance()->pmpro_select_list_options($saved_integration);
    ...
    <?php else : ?>
        ...
        if ( ! empty($saved_list)) {
            ...
            $mappable_fields = PMPROInit::get_instance()->merge_vars_field_map($saved_integration, $saved_list);
    ...
    public function process_submission(\MemberOrder $morder)
    {
        ...
        foreach (PMPROInit::merge_vars_field_map($connection, $connection_email_list) as $key => $value) {
