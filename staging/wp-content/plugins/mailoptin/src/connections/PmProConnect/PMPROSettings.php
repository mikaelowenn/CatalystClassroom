...
class PMPROSettings
{
    ...
    public function pmpro_settings($settings)
    {
        ...
        if ( ! defined('MAILOPTIN_DETACH_LIBSODIUM')) :
            ...
            if ( ! empty($saved_connections) && $saved_connections != 'leadbank') {
                ...
                $lists                                               = PMPROInit::get_instance()->pmpro_select_list_options($saved_connections);
                ...
                if ( ! empty($saved_lists)) {
                    $mappable_fields = PMPROInit::get_instance()->merge_vars_field_map($saved_connections, $saved_lists);
    ...
    public function process_submission(\MemberOrder $morder)
    {
        ...
        foreach (PMPROInit::get_instance()->merge_vars_field_map($connection_service, $connection_email_list) as $key => $value) {
            $mapped_key      = rawurlencode('mailoptin_pmpro_mapped_fields_' . $key);
