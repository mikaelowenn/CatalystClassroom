...
class LearnDashSettings
{
    ...
    public function learndash_settings($settings)
    {
        ...
        if ( ! defined('MAILOPTIN_DETACH_LIBSODIUM')) :
            ...
            if ( ! empty($saved_connections) && $saved_connections != 'leadbank') {
                ...
                $lists                                                       = LearnDashInit::get_instance()->learndash_select_list_options($saved_connections);
                ...
                if ( ! empty($saved_lists)) {
                    $mappable_fields = LearnDashInit::get_instance()->merge_vars_field_map($saved_connections, $saved_lists);
    ...
    public function process_submission($course_id, $user_id)
    {
        ...
        foreach (LearnDashInit::get_instance()->merge_vars_field_map($connection_service, $connection_email_list) as $key => $value) {
            $mapped_key      = rawurlencode('mailoptin_learndash_mapped_fields_' . $key);
