...
class Settings
{
    ...
    public function settings($settings)
    {
        ...
        if (defined('MAILOPTIN_DETACH_LIBSODIUM')) {
            ...
            if ( ! empty($saved_connections)) {
                ...
                $lists = EDDInit::get_instance()->edd_select_list_options($saved_connections);
                ...
                if ( ! empty($saved_lists)) {
                    foreach (EDDInit::get_instance()->merge_vars_field_map($saved_connections, $saved_lists) as $key => $value) {
    ...
    public function process_submission($payment_meta)
    {
        ...
        foreach (EDDInit::get_instance()->merge_vars_field_map($connection_service, $connection_email_list) as $key => $value) {
            $mapped_key      = rawurlencode('mailoptin_edd_mapped_fields-' . $key);
