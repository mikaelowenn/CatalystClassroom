...
class Downloads
{
        ...
        if (isset($integration)) {
            ...
            if ( ! empty($integration)) {
                if (isset($_POST['mailoptinEDDSelectList']) && isset($_POST['mailoptinEDDSelectIntegration'])) {
                    ...
                    foreach (EDDInit::get_instance()->merge_vars_field_map($connection, $connection_email_list) as $key => $value) {
    ...
    public function process_submission($download_id, $payment_meta)
    {
        ...
        foreach (EDDInit::merge_vars_field_map($connection, $connection_email_list) as $key => $value) {
            $mapped_key       = rawurlencode('mailoptinEDDMappedFields-' . $key);
