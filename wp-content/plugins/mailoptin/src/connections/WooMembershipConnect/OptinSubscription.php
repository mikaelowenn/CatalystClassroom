...
class OptinSubscription
{
     ...
     * @return array
     */
    public function woo_select_list_options($saved_integration)
    {
        $lists = [];
     ...
     * @return array
     */
    public function merge_vars_field_map($saved_integration, $saved_list)
    {
        $field_map = [];
    ...
    public function fetch_lists()
    {
        ...
        if ( ! empty($connection) && $connection != 'leadbank') {
            $lists       = $this->woo_select_list_options($connection);
    ...
    public function save_mailoptin_integration($post_id)
    {
        ...
        if ( ! empty($_POST['mailoptinWooMemSelectList']) && ! empty($_POST['mailoptinWooMemSelectIntegration'])) {
            ...
            foreach (WooInit::get_instance()->merge_vars_field_map($connection, $connection_email_list) as $key => $value) {
                $mapped_key = rawurlencode('mailoptinWooMemMappedFields-' . $key);
