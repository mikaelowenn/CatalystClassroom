...
class MemberPressSettings
{
    ...
    public function fetch_lists()
    {
        ...
        if ( ! empty($connection) && $connection != 'leadbank') {
            $lists       = MemberPressInit::get_instance()->memberpress_select_list_options($connection);
    ...
    public function fetch_custom_fields()
    {
        ...
        $mappable_fields = MemberPressInit::get_instance()->merge_vars_field_map($connection, $connection_email_list);
    ...
    public function process_submission($connection, $user_data)
    {
        ...
        foreach (MemberPressInit::merge_vars_field_map($connection, $connection_email_list) as $key => $value) {
            $mapped_key       = rawurlencode('mailoptinMemberPressMappedFields-' . $key);
        ...
        if (isset($integration)) {
            ...
            if ( ! empty($integration)) {
                if (isset($_POST['mailoptinMemberPressSelectList']) && isset($_POST['mailoptinMemberPressSelectIntegration'])) {
                    ...
                    foreach (MemberPressInit::get_instance()->merge_vars_field_map($connection, $connection_email_list) as $key => $value) {
                        $mapped_key = rawurlencode('mailoptinMemberPressMappedFields-' . $key);
