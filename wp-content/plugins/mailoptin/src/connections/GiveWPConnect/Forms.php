...
class Forms
{
        ...
        if (isset($integration)) {
            ...
            if ( ! empty($integration)) {
                if (isset($_POST['mailoptinGWPSelectList']) && isset($_POST['mailoptinGWPSelectIntegration'])) {
                    ...
                    foreach (GWPInit::get_instance()->merge_vars_field_map($connection, $connection_email_list) as $key => $value) {
    ...
    public function process_submission($payment)
    {
        ...
        foreach (GWPInit::merge_vars_field_map($connection, $connection_email_list) as $key => $value) {
            $mapped_key       = rawurlencode('mailoptinGWPMappedFields-' . $key);
