...
class Membership
{
    ...
    public function membership_level_settings($level = false)
    {
        ...
        $lists      = RCPInit::get_instance()->rcp_select_list_options($saved_integration);
    ...
    <?php else : ?>
        ...
        if ( ! empty($saved_list)) {
            ...
            $mappable_fields = RCPInit::get_instance()->merge_vars_field_map($saved_integration, $saved_list);
    ...
    public function process_submission(\RCP_Membership $membership)
    {
        ...
        foreach (RCPInit::merge_vars_field_map($connection, $connection_email_list) as $key => $value) {
