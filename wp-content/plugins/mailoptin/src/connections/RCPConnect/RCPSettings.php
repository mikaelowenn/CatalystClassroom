...
class RCPSettings
{
    ...
    public function rcp_settings($settings)
    {
        ...
        if ( ! defined('MAILOPTIN_DETACH_LIBSODIUM')) :
            ...
            if ( ! empty($saved_connections) && $saved_connections != 'leadbank') {
                ...
                $lists                                           = RCPInit::get_instance()->rcp_select_list_options($saved_connections);
                ...
                if ( ! empty($saved_lists)) {
                    $mappable_fields = RCPInit::get_instance()->merge_vars_field_map($saved_connections, $saved_lists);
    ...
    public function process_submission(\RCP_Membership $membership)
    {
        ...
        foreach (RCPInit::get_instance()->merge_vars_field_map($connection_service, $connection_email_list) as $key => $value) {
            $mapped_key      = rawurlencode('mailoptin_rcp_mapped_fields_' . $key);
