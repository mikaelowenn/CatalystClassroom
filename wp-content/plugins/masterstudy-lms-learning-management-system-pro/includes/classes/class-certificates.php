...
*/
	public static function generate_certificate_user_code( $user_id, $course_id ) {
		...
		$request_create = "INSERT INTO `{$table}` VALUES (DEFAULT, DEFAULT, DEFAULT, $user_id, $course_id);";
		$wpdb->get_results( $request_create, ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared

		$request_get = "SELECT *, CONCAT(LPAD(id, 4, '0'), '/SC-CC/CERT/', month, '/', year) cert_id FROM {$table} WHERE `id`= LAST_INSERT_ID()		";
		$cert = STM_LMS_Helpers::simplify_db_array( $wpdb->get_results( $request_get, ARRAY_A ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
