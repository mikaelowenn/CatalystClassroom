	...
	<div class="stm-lms-buy-buttons stm-lms-buy-buttons-mixed stm-lms-buy-buttons-mixed-pro">
		<?php if ( ( $has_course || ( empty( $course_price ) && ! $not_salebale ) ) && $is_prerequisite_passed ) : ?>
			...
			if ( empty( $user['id'] ) ) :
				...
				<a href="/sign-in?redirect_to=<?php echo urlencode($_SERVER['REQUEST_URI']) ?>" class="btn btn-default">
			...
			<div class="<?php echo esc_attr( implode( ' ', $mixed_classes ) ); ?>">
				...
				<div class="stm_lms_mixed_button__list">
					...
					if ( $dropdown_enabled ) :
						...
						if ( ! $not_salebale && empty( $uid ) ) :
							?>
							<a class="stm_lms_mixed_button__single" href="/sign-in?redirect_to=<?php echo urlencode($_SERVER['REQUEST_URI']) ?>">
