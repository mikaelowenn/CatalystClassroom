...
class GP_Auto_Login extends GP_Plugin {
	...
	public function init() {
		...
		add_filter( 'gform_is_feed_asynchronous', array( $this, 'ensure_feed_is_sync' ), 10, 2 );
	...
	/**
	 * Disable async user registration feeds for User Registration 5.0.1+ and GF 2.7.1+ as the process
	 * needs to be synchronous to automatically login.
	 *
	 ...
	 * @return bool
	 */
	public function ensure_feed_is_sync( $is_asynchronous, $feed ) {
		if ( rgar( $feed, 'addon_slug' ) !== 'gravityformsuserregistration' || ! $is_asynchronous ) {
			return $is_asynchronous;
		}

		return $this->is_auto_login_enabled( $feed ) ? false : $is_asynchronous;
	}
